...
int AnnealingHamerlyKmeans::runThread(int threadId, int maxIterations) {
    int iterations = 0;

    double changedLastIter = 0.0;
    ...
    while ((iterations < maxIterations) && ! converged) {
        ...
        for (int i = startNdx; i < endNdx; ++i) {
            ...
            if (upper[i] / atr <= upper_comparison_bound) {
                continue;
            }
        ...
        maxChanged = std::max(maxChanged, changedLastIter);
 ...
 */
void AnnealingHamerlyKmeans::update_bounds(int startNdx, int endNdx) {
